
---


### 🌳 Starting Point (more math, theory + practice)


#### 1. Deep Learning Specialisation (Andrew Ng)

* **Type:** Video course + notebooks
* **Link:** [https://www.coursera.org/specializations/deep-learning](https://www.coursera.org/specializations/deep-learning)
* **Why it’s good:**

  * Excellent place to start the Deep Learning journey
  * Good and gentle coverge and balance of math and practical focus
* **Best for:** Students with ML experience/ knowledge

---
#### 2. **fast.ai – Practical Deep Learning for Coders**

* **Type:** Video course + notebooks
* **Link:** [https://course.fast.ai/](https://course.fast.ai/)
* **Why it’s good:**

  * Learn by building real models early
  * Minimal math at first, practical focus
* **Best for:** Students who learn best by doing





### 🌲 Advanced / Research-Oriented

#### 1. **Deep Learning Book (Ian Goodfellow et al.)**

* **Type:** Free online textbook
* **Link:** [https://www.deeplearningbook.org/](https://www.deeplearningbook.org/)
* **Why it’s good:**

  * Definitive deep learning reference
  * Very theoretical
* **Best for:** Students and Researchers
* **[Two Minute Papers](https://www.youtube.com/user/keeroyz)** – Short, engaging summaries of cutting-edge AI/ML research and breakthroughs. 




---
### Other Resources 
#### 1. **Kaggle Learn**

* **Type:** Datasets + micro-courses
* **Links:**

  * [https://www.kaggle.com/learn](https://www.kaggle.com/learn)

* **Why they’re good:**

  * Practice on real datasets
  * Free beginner → advanced content
  * Run experiments and code online (no installation is needed)
* **Best for:** Applying theory to real problems

#### 2. **Hugging Face**

* **Type:** Datasets + micro-courses
* **Links:**

  * [https://huggingface.co/learn](https://huggingface.co/learn)

* **Why they’re good:**

  * Practice on real datasets
  * Free beginner → advanced content
  * Run experiments and code online (no installation is needed)
* **Best for:** NLP, Computer Vision and Transformers

---


[Back to AI Journey](../README.md)
